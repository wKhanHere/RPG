...
<<<<<<<< HEAD:RRPG/ItemsCore/ItemsDEPRECATED.py
#*1 Overwrites parent code, find a way to overload/copy and paste parent code here. -> Solved, Call super().<func>() in <func>()
#2 Really isn't a problem, just an idea to reduce work on computer by checking for Consuming logic only for consumables.
...
#*2.5 Created bool "usable_item" to be checked for before running, if False, useItem code is aborted. (Aborted in child class, to give the generic cd/count msg from Parent)
...
list of "to be added" artifacts via names internally mapped (hardcoded, can be changed
...
#$4 With the above fix also using uuids now from UUID module for each artifact, how am I supposed to refer to the objects via these ids? (Maybe another UUID Dictionary?)
#+5 Switch to Snake Case
#+6 Does anything other than artifacts really need UUIDs?
#7 The only reason I might need usable item tag is to list on use effects in Menus, and to sort by usable items in bags.
import random, uuid #Use uuid.uuid4() or uuid.uuid1(). The Latter uses Comp's ip and stuff, so security risk, don't.
class Item:
    """Defines an Item object class which contains data about amount, effects and use triggers."""
    def __init__(self):
        self.Name:str = "<Name>"
        self.ItemId:str = "item:placeholder"
    ...
    def UseItem(self):
        """Under item class, only implements item existence and cooldown check."""
========
...
>>>>>>>> cda2f2a (Datatype fixes):RRPG/ItemsCore/Items.py
...
<<<<<<<< HEAD:RRPG/ItemsCore/ItemsDEPRECATED.py
        self.Uuid = "FALLBACKUUID" # a -> Artifact
...
Player1 = Person()
ValidArtiList = ["fate_die","invalid_piece"]
ArtiGivenList = ["fate_die","fate_die","invalid_piece","fate_die"] #artifacts to add
========
    del(void_call) #Saves Space Ig.
Player1 = Person()
...
<<<<<<<< HEAD:RRPG/ItemsCore/ItemsDEPRECATED.py
    Artif:Artifact = Player1.InvArti[choice_idx] #Place holder code ofc
    # call with player and index
    Artif.UseItem(Player1, choice_idx)
========
